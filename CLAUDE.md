...
- **Navigation:** user-facing **application** modules belong in `sidebar` as their standard entry point. **System/admin tooling** (integrations, webhooks, backup, health, scheduler, …) does **not** go in the top-level `sidebar`: expose it through the admin catalog by declaring an `admin_panel` block in the module's `module.json` (auto-discovered by `App\Modules\Admin\Services\AdminIndexService`). Only Admin's own native `admin.*` routes are hardcoded in that service. Do not add application modules to `user_menu` (header dropdown) or `radial`: both are reserved as personal user zones / core shortcuts.
...
- **`docs/contracts/`** — platform contracts by topic, loaded on demand:
  ...
  - [integrations.md](docs/contracts/integrations.md) — reusable providers (notifications, dashboard, search, export, contacts, calendar, scheduler) + outward integrations (public API token seam, outgoing webhooks, Web Push channel).
  - **[docs/api/](docs/api/)** — outward-facing integration docs: [README.md](docs/api/README.md) (developer reference: API auth, envelopes, webhook HMAC verification, Web Push) + [openapi.json](docs/api/openapi.json) (OpenAPI 3.1, source of truth for the endpoint catalog).
