...
- **Navigation:** user-facing **application** modules belong in `sidebar` as their standard entry point. **System/admin tooling** (integrations, webhooks, backup, health, scheduler, …) does **not** go in the top-level `sidebar`: expose it through the admin catalog (`App\Modules\Admin\Services\AdminIndexService` — hardcoded for core system tooling, or an `admin_panel` block in `module.json` for a module's own admin console). Do not add application modules to `user_menu` (header dropdown) or `radial`: both are reserved as personal user zones / core shortcuts.
