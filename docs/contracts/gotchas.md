...
| Hardcoded admin panel for a **module** | a module's own links written directly in `AdminIndexService::catalog()` | give the module an `admin_panel` in its `module.json` (auto-discovered — this also makes its row clickable in Admin → Moduli via `ModuleManagementService::resolveAdminLink()`). Target an existing section title via `admin_panel.section` to keep it grouped. `catalog()` hardcodes **only Admin-native routes** (`admin.*`); every other module — app or system tooling (Backup, Scheduler, HealthCheck, Reports, Notifications, Webhooks) — self-declares via `admin_panel` |
